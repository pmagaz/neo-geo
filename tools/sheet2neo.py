...
def snap_to_neogeo(rgb):
    """Round a colour to what the Neo Geo can display: 5 bits per channel."""
...
def to_color_word(r, g, b):
    ...
    Bit 15 is a "dark" bit shared by all three channels, stored inverted;
    bits 14-12 hold each channel's low bit; bits 11-0 hold four bits each of
    red, green and blue.
    """
...
def main():
    ...
    for y in range(out.height):
        for x in range(out.width):
            r, g, b, alpha = px[x, y]
            px[x, y] = snap_to_neogeo((r, g, b)) + (255,) if alpha > 127 else (0, 0, 0, 0)
